...
static int rsa2_pss_sha256_verifysig(void *key, char *sig, int siglen, char *data, int datalen)
{
	struct RSAKey *rsa = (struct RSAKey *) key;
	Bignum in, out;
	...
	if (em[bytes - 1] != 0xbc) {
		...
		if (ret) {
			...
			if (memcmp(hash, hash2, hashLen)) {
				...
				//Decryption
				if (slen > 32 && memcmp(data + 0x240, magic, 20)) {
					struct AES_key aes;
					AES_set_decrypt_key(salt, 128, &aes);
					AES_cbc_encrypt(data + 0x230 - 20, data + 0x230 - 20, (datalen - 0x230 + 20) & ~0xF, &aes, salt + 16, 0);
				}
			}
		}
	}
	...
	unsigned int pss_mode)
{
	...
	if (pss_mode) {
		re = rsa2_pss_sha256_verifysig(rsa, signBuf, in_Signature_len, (char *)in_data, in_data_len);
	} else {
